    ...
    <div className="space-y-8 pb-16 animate-fade">
      ...
      <div className="flex items-center justify-between bg-card p-4 rounded-xl border border-border shadow-subtle">
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2 px-3 py-2 bg-background border border-border rounded-lg cursor-pointer group hover:border-primary transition-all">
            <Filter size={16} className="text-text-muted group-hover:text-primary" />
            <span className="text-xs font-bold text-text-primary uppercase tracking-wider">Property: All Buildings</span>
            <ChevronDown size={14} className="text-text-muted" />
          </div>
          <div className="flex items-center gap-2 px-3 py-2 bg-background border border-border rounded-lg cursor-pointer group hover:border-primary transition-all">
            <Calendar size={16} className="text-text-muted group-hover:text-primary" />
            <span className="text-xs font-bold text-text-primary uppercase tracking-wider">Range: Last 30 Days</span>
            <ChevronDown size={14} className="text-text-muted" />
          </div>
        </div>
        <div className="flex items-center gap-3">
          <button className="flex items-center gap-2 px-4 py-2 bg-primary text-white rounded-lg text-[11px] font-black uppercase tracking-widest shadow-md shadow-primary/20 hover:bg-primary-dark transition-all">
      ...
      <div className="card-classic p-8">
         ...
         <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-6">
            {[
              { label: 'Add Booking', icon: <CalendarCheck />, color: 'indigo' },
              { label: 'Add Tenant', icon: <UserPlus />, color: 'emerald' },
              { label: 'Create Invoice', icon: <FileText />, color: 'blue' },
              { label: 'Add Complaint', icon: <AlertTriangle />, color: 'rose' },
              { label: 'Room Map', icon: <Map />, color: 'violet' },
              { label: 'Maintenance', icon: <Wrench />, color: 'orange' },
              { label: 'Reports', icon: <BarChart3 />, color: 'cyan' },
              ...
              >
                 <div className={`w-14 h-14 rounded-2xl bg-${action.color}-500/10 text-${action.color}-500 border border-${action.color}-500/20 flex items-center justify-center group-hover:shadow-glow group-hover:bg-${action.color}-500 group-hover:text-white transition-all duration-300`}>
                    {React.cloneElement(action.icon, { size: 24, strokeWidth: 2.5 })}
                 </div>
                 <span className="text-[11px] font-black text-text-muted uppercase tracking-tight group-hover:text-text-primary transition-colors text-center">{action.label}</span>
              </motion.button>
            ))}
         </div>
      </div>
          ...
          <div className="h-[340px]">
             <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={REVENUE_DATA} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
                  ...
                  <Area type="monotone" dataKey="revenue" stroke="rgb(var(--primary))" strokeWidth={4} fillOpacity={1} fill="url(#colorRev)" />
