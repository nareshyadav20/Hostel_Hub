...
const Tenants = () => {
  ...
  const stats = [
    ...
    },
    {
      ...
      value: tenants.filter(t => t.status && t.status.toLowerCase() === 'active').length,
      change: '+5.4%',
      icon: UserCheck,
      description: 'Verified Stay',
    ...
    },
    {
      ...
      value: `₹${(tenants.filter(t => t.rentStatus && t.rentStatus.toLowerCase() === 'paid').length * 8.5).toFixed(1)}L`,
      change: '+12.5%',
      icon: DollarSign,
      description: 'MTD Revenue',
  ...
  return (
    <div className="space-y-10 pb-20 animate-fade">
        ...
        {displayedStats.map((stat, i) => {
          ...
          return (
            ...
            >
              ...
              <p className="relative text-[10px] text-text-muted mt-0.5">{stat.description}</p>
            </motion.div>
          );
        })}
      </div>
      ...
      {/* --- DIRECTORY FOOTER --- */}
      <div className="mt-10 flex flex-col sm:flex-row items-center justify-between gap-6 p-6 bg-surface border border-divider rounded-2xl shadow-sm">
        <div className="flex items-center gap-4">
