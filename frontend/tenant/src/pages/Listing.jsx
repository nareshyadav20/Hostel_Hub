...
const Listing = () => {
  ...
  useEffect(() => {
    const fetchHostelDetails = async () => {
      ...
      try {
        ...
        if (mapped.roomTypes.length === 0) mapped.roomTypes = [{ type: '101' }, { type: '102' }];
        setHostel(mapped);
  ...
  if (loading) return <div style={{ display: 'flex', height: '100vh', alignItems: 'center', justifyContent: 'center', color: '#10B981', fontWeight: '800' }}>Crafting your premium stay...</div>;
      ...
      <main style={{ maxWidth: '1400px', margin: '0 auto' }}>
        ...
        <section style={{ background: 'white', borderRadius: '32px', padding: '3rem', border: '1px solid #E2E8F0', marginBottom: '2rem', position: 'relative', boxShadow: '0 10px 30px rgba(0,0,0,0.02)' }}>
          <div style={{ position: 'absolute', top: '2.5rem', right: '2.5rem', display: 'flex', gap: '1.2rem', zIndex: 10 }}>
            <button onClick={handleShare} style={{ background: '#F1F5F9', border: 'none', padding: '0.8rem 1.6rem', borderRadius: '14px', fontWeight: '800', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.6rem', color: '#64748B' }}>
              <ICONS.Share /> Share
            </button>
            <button onClick={handleWishlistToggle} style={{ background: wishlistId ? '#EF4444' : '#F1F5F9', border: 'none', padding: '0.8rem 1.6rem', borderRadius: '14px', fontWeight: '800', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.6rem', color: wishlistId ? 'white' : '#64748B' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: '4rem', alignItems: 'center' }}>
            ...
            <div style={{ background: '#F8FAFC', padding: '3.5rem', borderRadius: '36px', border: '1px solid #E2E8F0', textAlign: 'center', boxShadow: 'inset 0 4px 12px rgba(0,0,0,0.03)' }}>
              ...
              <button onClick={() => navigate(`/booking/${hostel.id}`)} style={{ width: '100%', background: '#10B981', color: 'white', border: 'none', padding: '1.6rem', borderRadius: '22px', fontWeight: '950', fontSize: '1.5rem', cursor: 'pointer', boxShadow: '0 15px 35px rgba(16, 185, 129, 0.25)' }}>Book Now</button>
            </div>
          </div>
        </section>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '3.5rem' }}>
            ...
            <section style={{ background: 'white', borderRadius: '36px', padding: '4rem', border: '1px solid #E2E8F0', boxShadow: '0 4px 25px rgba(0,0,0,0.02)' }}>
              ...
              <div style={{ background: '#F8FAFC', borderRadius: '32px', padding: '3rem', marginBottom: '4rem', border: '1px solid #E2E8F0' }}>
                ...
                <p style={{ margin: '1.2rem 0 0', color: '#64748B', fontWeight: '600', lineHeight: '1.8', fontSize: '1.2rem' }}>Spacious premium suite with designer work pods, ergonomic storage solutions, and high-performance climate control.</p>
                ...
                <div style={{ marginTop: '3rem' }}>
                  ...
                  <div style={{ display: 'flex', gap: '1.5rem', flexWrap: 'wrap' }}>
                    {[...Array(selectedSharing)].map((_, i) => (
                      <button key={i} disabled={i === 0} style={{ padding: '1.2rem 2.5rem', borderRadius: '18px', background: i === 0 ? '#EF4444' : '#10B981', color: 'white', border: 'none', fontWeight: '950', fontSize: '1.1rem', cursor: i === 0 ? 'not-allowed' : 'pointer', opacity: i === 0 ? 0.8 : 1, boxShadow: `0 10px 20px ${i === 0 ? 'rgba(239, 68, 68, 0.2)' : 'rgba(16, 185, 129, 0.2)'}` }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '1.8rem' }}>
                {dynamicSpecs.map(spec => (
                  <div key={spec.label} onClick={() => setActiveSpec(spec.label)} style={{ background: activeSpec === spec.label ? '#EFF6FF' : 'white', borderRadius: '32px', padding: '2.2rem', border: `2px solid ${activeSpec === spec.label ? '#3B82F6' : '#F1F5F9'}`, textAlign: 'center', cursor: 'pointer' }}>
            ...
            <section style={{ background: 'white', borderRadius: '36px', padding: '4rem', border: '1px solid #E2E8F0' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '2rem' }}>
                {hostel.plans.map((p, i) => (
                  <div key={p.name} onClick={() => setSelectedPlanIdx(i)} style={{ padding: '2.8rem', borderRadius: '36px', border: `4px solid ${selectedPlanIdx === i ? '#10B981' : '#F1F5F9'}`, background: selectedPlanIdx === i ? '#F0FDF4' : 'white', cursor: 'pointer' }}>
          ...
          <aside style={{ display: 'flex', flexDirection: 'column', gap: '3rem' }}>
            <div style={{ background: 'white', borderRadius: '36px', padding: '3.5rem', border: '1px solid #E2E8F0', boxShadow: '0 40px 80px rgba(0,0,0,0.06)', position: 'sticky', top: '2rem' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1.8rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontWeight: '850', color: '#64748B', fontSize: '1.25rem' }}>Monthly Rent <span>₹{hostel.price.toLocaleString()}</span></div>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontWeight: '850', color: '#64748B', fontSize: '1.25rem' }}>Security Deposit <span>₹{hostel.deposit.toLocaleString()}</span></div>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', fontWeight: '850', color: '#64748B', fontSize: '1.25rem' }}>Maintenance Fee <span>₹{hostel.maintenance}</span></div>
                <div style={{ borderTop: '5px dashed #F1F5F9', paddingTop: '2.5rem', marginTop: '1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <span style={{ fontSize: '3.2rem', fontWeight: '950', color: '#10B981', letterSpacing: '-1.5px' }}>₹{(hostel.price + hostel.deposit + activePlan.price + hostel.maintenance).toLocaleString()}</span>
                </div>
              </div>
              <button onClick={() => navigate(`/booking/${hostel.id}`)} style={{ width: '100%', background: '#10B981', color: 'white', border: 'none', padding: '1.8rem', borderRadius: '24px', fontWeight: '950', fontSize: '1.6rem', marginTop: '4rem', cursor: 'pointer', boxShadow: '0 25px 50px rgba(16, 185, 129, 0.3)' }}>Reserve This Bed</button>
              <button onClick={handleWishlistToggle} style={{ width: '100%', background: 'white', color: '#0F172A', border: '3px solid #F1F5F9', padding: '1.5rem', borderRadius: '24px', fontWeight: '950', marginTop: '1.5rem', cursor: 'pointer', fontSize: '1.3rem' }}>Schedule Site Visit</button>
            </div>
          </aside>
        </div>
      </main>
