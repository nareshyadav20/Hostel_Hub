...
const NotificationToast = ({ notification, onClose }) => {
  ...
  return (
    <div className={`notif-toast-premium ${isVisible ? 'show' : 'hide'} priority-${notification.priority?.toLowerCase() || 'medium'}`}>
      <div className="toast-icon">
        {notification.priority === 'High' ? '🚨' : getIcon(notification.category)}
      </div>
      <div className="toast-content">
        <div className="toast-header">
          <span className="toast-module">{notification.moduleName}</span>
          <span className="toast-priority-badge">{notification.priority}</span>
        </div>
